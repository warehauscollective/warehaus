...
/** Corner keys — kept here (not imported from Bevel) to avoid a circular module edge. */
...
/** Read the inspector from any <Bevel>. Returns a disabled no-op outside a provider. */
...
/**
 * Shared Bevel inspector provider. Apps may pass an optional `panel` slot
 * (e.g. Leva controls on the website). Portal uses the toolbar-only default.
 */
export function BevelInspectorProvider({
  children,
  panel,
}: {
  children: ReactNode;
  /** Optional editing panel rendered while the inspector is enabled. */
  panel?: ReactNode;
}) {
  if (!DEV) return <>{children}</>;
  return <InspectorInner panel={panel}>{children}</InspectorInner>;
}
